...
public class MainDialog {
    ...
    private enum AddAction { SKIP, ADD_RENAME, ADD_NONE, OVERWRITE };
    ...
    protected void openCombatData() {
        ...
        if (file != null) {
            try {
                ...
                Iterator<MythrasCombatant> i = combatants.iterator();
                while (i.hasNext() && addList) {
                    MythrasCombatant m = i.next();
                    if (mythrasCombatModel.combatants.contains(m)) {
                        AddAction addAction = handleCombatantConflict(m);
                        switch (addAction) {
                            case SKIP:
                                i.remove();
                                break;
                ...
                if (addList) mythrasCombatModel.combatants.addAll(combatants);
    ...
    private AddAction handleCombatantConflict(MythrasCombatant m) {
        AddAction result;

        Alert alert = new Alert(Alert.AlertType.CONFIRMATION);
        alert.setTitle("Duplicate Combatant");
        alert.setHeaderText("A combatant with the name " + m.getName() + " already exists in the combat.");
        ...
        if (alertResult.get() == buttonTypeOne){
            result = AddAction.SKIP;
        } else if (alertResult.get() == buttonTypeThree) {
            result = AddAction.OVERWRITE;
        } else {
            result = AddAction.ADD_NONE;
        }
    ...
    protected MenuBar addMenu() {
        ...
        MenuItem menuDeleteCombatant = new MenuItem("Remove Combatant");
        menuDeleteCombatant.setAccelerator(new KeyCodeCombination(KeyCode.R, systemKey));
        ...
        MenuItem menuDeleteAllCombatants = new MenuItem("Remove All Combatants");
        menuDeleteAllCombatants.setOnAction( event -> {
            mythrasCombatModel.combatants.clear();
            listView.refresh();
        });

        MenuItem menuDeleteNpcs = new MenuItem("Remove NPCs");
        ...
        menuCombatant.getItems().addAll(menuResetActionPoints, menuDeleteCombatant, menuDeleteNpcs, menuDeleteAllCombatants,
